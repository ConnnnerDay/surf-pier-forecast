...
def search_regulations(query: str, state: str) -> List[Dict]:
    """Search fishing regulations for a state, optionally filtered by species name.

    Returns a list of matches sorted alphabetically by display name.  Each
    entry contains ``display_name``, ``species_key``, and the full regulation
    payload (same shape as ``lookup_regulation``).

    Only species that have snapshot data for the requested state are returned;
    the live scraper is not consulted here to keep results fast.
    """
    state_key = (state or "").upper().strip()
    if not state_key:
        return []
    ...
    # Build a reverse map from species_key → canonical display name.
    key_to_display: Dict[str, str] = {}
    for display_name, key in _REG_DATA.name_map.items():
        if key not in key_to_display:
            key_to_display[key] = display_name

    q = (query or "").strip().lower()
    base = _base_payload(state_key)

    results = []
    for species_key, details in state_regs.items():
        display_name = key_to_display.get(species_key, species_key.replace("_", " ").title())

        # Filter: check query against display name and key.
        if q and q not in display_name.lower() and q not in species_key.replace("_", " "):
            continue
