...
class TestOsmTiles:
    ...
    def test_tile_url_has_placeholders(self):
        ...
        cfg = get_tile_config()
        osm = next(layer for layer in cfg["layers"] if layer["id"] == "osm_standard")
        assert "{z}" in osm["url"]
        assert "{x}" in osm["url"]
        assert "{y}" in osm["url"]
...
class TestNaturalEarth:
    def test_get_coastlines_returns_feature_collection_on_download_failure(
        ...
        features = [
            {
                "geometry": {
                    "type": "Polygon",
                    "coordinates": [
                        [[-75.0, 36.0], [-74.5, 36.0], [-74.5, 36.5], [-75.0, 36.0]]
                    ],
                },
...
class TestAerialImagery:
    def test_get_aerial_tile_config_structure(self):
        ...
        esri_ids = {layer["id"] for layer in cfg["layers"]}
    ...
    def test_esri_imagery_url_valid(self):
        ...
        cfg = get_aerial_tile_config()
        esri = next(
            layer for layer in cfg["layers"] if layer["id"] == "esri_world_imagery"
        )
...
class TestHdxFao:
    ...
    def test_get_hdx_fao_enrichment_structure(self):
        ...
        with (
            patch(
                "services.hdx_fao.fetch_fao_fisheries_zones",
                return_value={
                    "area_code": "21",
                    "area_name": "Northwest Atlantic",
...
class TestGeoApiBlueprint:
    ...
    def test_geo_environmental_valid_params(self, client):
        with (
            patch(
                "web.geo_api.get_water_quality_summary",
                return_value={"available": False, "source": "EPA"},
            ),
            patch(
                "web.geo_api.get_sst_tile_config",
                return_value={
                    "url": "https://gibs.example.com/{z}/{y}/{x}.png",
                    "options": {},
                },
            ),
    ...
    def test_geo_coastlines_endpoint(self, client):
        with patch(
            "web.geo_api.get_coastlines_geojson",
    ...
    def test_geo_esri_piers_valid_bbox(self, client):
        with patch("web.geo_api.fetch_pier_locations", return_value=[]):
            resp = client.get(
                "/api/v1/geo/esri/piers?south=35&west=-76&north=37&east=-74"
            )
        assert resp.status_code == 200
    ...
    def test_geo_hdx_fao_valid_params(self, client):
        with patch(
            "web.geo_api.get_hdx_fao_enrichment",
            return_value={
                "available": True,
                "fao_zone": {},
                "hdx_datasets": [],
