...
def test_v1_regulations_extended_nc_species_have_guidance(client):
    """Additional NC species should return guidance rows instead of empty fallback payloads."""
    resp = client.get("/api/v1/regulations?species=Northern+puffer+%28blowfish%29&state=NC")
    assert resp.status_code == 200
    body = resp.get_json()
    reg = body["data"]["regulation"]
    assert reg is not None
    assert reg["data_status"] == "snapshot"
    assert reg["season"] == "Varies by species and area"
