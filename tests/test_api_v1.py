...
from storage.sqlite import create_user, init_db, set_user_tier
...
def test_v1_log_feature_locked_for_free(client):
    uid = create_user("apiv1_free", "pass1234", email="free@example.com")
    assert uid is not None
    _login_session(client, uid)

    resp = client.get("/api/v1/log?location_id=wrightsville-beach-nc")
